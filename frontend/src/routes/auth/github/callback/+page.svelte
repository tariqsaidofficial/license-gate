<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			logSuccess('Successfully signed in with GitHub!')
			goto('/dashboard')
		} catch (err: any) {
			console.error('GitHub login error:', err)
			
			if (err.data?.code === 'no-account-for-github-email') {
				error = 'No account found for this GitHub email. Please sign up first.'
...
<svelte:head>
	<title>GitHub OAuth Callback - LicenseGate</title>
</svelte:head>
	...
	<div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
		{#if loading}
			<div class="text-center">
				...
				<h2 class="mt-4 text-xl font-semibold text-gray-700">Completing GitHub Sign In...</h2>
				<p class="mt-2 text-gray-500">Please wait while we process your authentication.</p>
			</div>
		{:else if error}
			<div class="text-center">
				...
				<h2 class="mt-4 text-xl font-semibold text-gray-700">Authentication Failed</h2>
