...
class Authenticator {
    ...
    loginWithPassword(passwordHash, password, data, existingRefreshSession) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!(yield argon2_1.default.verify(passwordHash, password)))
    ...
    refreshAccessToken(refreshToken, refreshSession) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { data, refreshSession: tokenRefreshSession } = jsonwebtoken_1.default.verify(refreshToken, this.secret);
                const session = yield refreshSession(data);
    ...
    generateMailToken(email) {
        return jsonwebtoken_1.default.sign({ email: email.toLowerCase() }, this.secret, {
            expiresIn: "36h",
        });
    }
    verifyMailToken(token, email) {
        try {
            const { email: tokenEmail } = jsonwebtoken_1.default.verify(token, this.secret);
    ...
    verifyAccessToken(accessToken) {
        try {
            const { data } = jsonwebtoken_1.default.verify(accessToken, this.secret);
            return {
                success: true,
                data,
            };
